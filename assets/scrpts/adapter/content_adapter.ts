 ...
 */
 ...
 export default class ContentAdapter extends Component {
     onLoad() {
         ...
         let srcScaleForShowAll = Math.min(view.getCanvasSize().width / this.node.width, view.getCanvasSize().height / tran.height);
         let realWidth = tran.width * srcScaleForShowAll;
         let realHeight = tran.height * srcScaleForShowAll;
 
         // 2. 基于第一步的数据，再做节点宽高适配
         tran.width = tran.width * (view.getCanvasSize().width / realWidth);
         tran.height = tran.height * (view.getCanvasSize().height / realHeight);
