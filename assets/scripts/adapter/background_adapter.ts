 ...
 */
 ...
 export default class BackgroundAdapter extends Component {
     onLoad() {
         ...
         let srcScaleForShowAll = Math.min(view.getCanvasSize().width / tran.width, view.getCanvasSize().height / tran.height);
         let realWidth = tran.width * srcScaleForShowAll;
         let realHeight = tran.height * srcScaleForShowAll;
 
         // 2. 基于第一步的数据，再做缩放适配
        
         let scale = Math.max(view.getCanvasSize().width / realWidth, view.getCanvasSize().height / realHeight);
